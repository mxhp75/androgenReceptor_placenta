...
# drop genes on the Y chromosome
...
# 1. estimate size factors (controlling for differences in the counts due varying sequencing depth of the samples)
# 2. and dispersions (The dispersion parameter captures how much the counts for the samples will vary around an expected value. Note that the expected value takes into consideration the sequencing depth and differences that can be attributed to variables in the design formula.)
# 3. Fitting a final generalized linear model using the size factors and dispersion values estimated above, which gives estimates of the log fold change.
dds <- DESeq(dds)
...
# topGene <- rownames(res_sex_pathology_Female_SGA_vs_Female_10_30)[which.min(res_sex_pathology_Female_SGA_vs_Female_10_30$padj)]
# 
# geneCounts <- plotCounts(dds, gene=topGene, intgroup=c("sex_pathology"), returnData=TRUE)
# ggplot(geneCounts, aes(x=sex_pathology, y=count, color = sex_pathology)) +
#   scale_y_log10() +
#   geom_point(position=position_jitter(width=.1,height=0), size=3)
# 
# ggplot(geneCounts, aes(x=sex_pathology, y=count, fill=sex_pathology)) +
#   scale_y_log10() +
#   geom_dotplot(binaxis = "y", stackdir = "center")
# 
# hist(res_sex_pathology_Female_SGA_vs_Female_10_30$pvalue[res_sex_pathology_Female_SGA_vs_Female_10_30$baseMean > 1], breaks=0:20/20, col="grey50", border="white")
...
```{r files for GSEA loop}

comparisons <- c("Male_AGA_vs_Female_AGA",
                 "Male_AGA_vs_Male_SGA",
                 "Male_AGA_vs_Female_SGA",
                 "Male_AGA_vs_Male_10_30",
                 "Male_AGA_vs_Female_10_30",
                 "Male_SGA_vs_Female_AGA",
                 "Male_SGA_vs_Female_SGA",
                 "Male_SGA_vs_Male_10_30",
                 "Male_SGA_vs_Female_10_30",
                 "Male_10_30_vs_Female_AGA",
                 "Male_10_30_vs_Female_SGA",
                 "Male_10_30_vs_Female_10_30",
                 "Female_AGA_vs_Female_SGA",
                 "Female_AGA_vs_Female_10_30",
                 "Female_SGA_vs_Female_10_30"
                 )
...
ARE_sites <- read.table(file = "~/Bioinformatics/github/postDoc_2022/androgenReceptor_placenta/cleanData/AREs.txt",
 ...
 # [5] "Genes.Positively.regulated.by.ARE.full.sites"                                         
 # [6] "Genes.Negatively.regulated.by.ARE.full.sites"  
 # [3] "Genes.Positively.regulated.by.ARE.half.sites"                                         
 # [4] "Genes.Negatively.regulated.by.ARE.half.sites"                                         

for (i in 1:length(comparisons)){
  
  # set which column are we interested in
  sites <- "Genes.Negatively.regulated.by.ARE.half.sites"
  # set path
  path <- "~/Bioinformatics/github/postDoc_2022/androgenReceptor_placenta/cleanData/negativeHalfSites/"
  # set the output file name for later when we save each table
  outputFilename <- paste(comparisons[i], sites, sep = "_") 
  # testing each file name
  # print(outputFilename)
  
  # copy the DE table and drop rows with NAs in the padj
  # note: DESeq2 will assign NA to the padj column if all samples in the comparison has zero counts for the gene
  # grab only the comparison we want
  comparison <- na.omit(list_of_datasets[[i]])
  
  # subset the table to only contain genes selected column of ARE_sites
  assign(paste0("comparison_", comparisons[i]),
         dplyr::filter(comparison,
                       gene %in% ARE_sites$Genes.Negatively.regulated.by.ARE.half.sites))
  
  # make the GSEA rank file
  gsea <- get(print(paste0("comparison_", comparisons[i]), quote = FALSE))
  # create a new column of GSEA ranking metric
  gsea["Rank_log"] <- (sign(gsea$log2FoldChange)) * (-log(gsea$padj, 10))
  gsea <- dplyr::arrange(gsea, desc(Rank_log))
  gsea_list <- gsea$Rank_log 
  names(gsea_list) <- gsea$gene
  
  write_delim(dplyr::select(gsea, gene, Rank_log),
              file = paste0(path, outputFilename, "_gseaRank", ".rnk"),
              delim = "\t")
...
### files for GSEA loop - genes _NOT_ in the ARE list
...
for (i in 1:length(comparisons)){
 ...
 # [1] "All.genes.containing.ARE.fullsites"                                                   
 # [2] "All.genes.containing.ARE.halfsites" 
  
  # set which column are we interested in
  sites <- "All.genes.containing.ARE.halfsites"
  # set path
  path <- "~/Bioinformatics/github/postDoc_2022/androgenReceptor_placenta/cleanData/notHalfSites/"
  # set the output file name for later when we save each table
  outputFilename <- paste(comparisons[i], sites, sep = "_") 
  # testing each file name
  # print(outputFilename)
  ...
  assign(paste0("comparison_", comparisons[i]),
         dplyr::filter(comparison,
                       gene %notin% ARE_sites$All.genes.containing.ARE.halfsites))
...
### files for GSEA individual

```{r files for GSEA}

# # copy the DE table and drop rows with NAs in the padj
# # note: DESeq2 will assign NA to the padj column if all samples in the comparison has zero counts for the gene
# comparison <- na.omit(Male_AGA_vs_Male_SGA)
# filename <- "Male_AGA_vs_Male_SGA"
# 
# # make the GSEA rank file
# gsea <- comparison
# 
...
# 
# write_delim(dplyr::select(gsea, gene, Rank_log),
#             file = paste0("~/Bioinformatics/github/postDoc_2022/androgenReceptor_placenta/cleanData/",
#                           filename, "_gseaRank", ".rnk"),
#             delim = "\t")


```

### ARE subset files for GSEA

```{r files for ARE subset GSEA}
...
#                         sep = "\t")
# 
# # set the file name for saved tables
# outputFilename <- paste0(comparisons[1], "_fullSites")
# 
# # copy the DE table and drop rows with NAs in the padj
# # note: DESeq2 will assign NA to the padj column if all samples in the comparison has zero counts for the gene
# comparison <- na.omit(Male_SGA_vs_Male_10_30)
# # subset the table to only contain genes with the full site
# comparison_ARE_full <- dplyr::filter(comparison,
#                                      gene %in% ARE_sites$Genes.Positively.regulated.by.ARE.full.sites)
# 
# # make the GSEA rank file
# gsea_fullSite <- comparison
# # create a new column of GSEA ranking metric
# gsea_fullSite["Rank_log"] <- (sign(gsea_fullSite$log2FoldChange)) * (-log(gsea_fullSite$padj,10))
# gsea_fullSite <- dplyr::arrange(gsea_fullSite, desc(Rank_log))
# gsea_fullSite_list <- gsea_fullSite$Rank_log 
# names(gsea_fullSite_list) <- gsea_fullSite$gene
# 
# write_delim(dplyr::select(gsea_fullSite, gene, Rank_log),
#             file = paste0("~/Bioinformatics/github/postDoc_2022/androgenReceptor_placenta/cleanData/",
#                           outputFilename, "_gseaRank", ".rnk"),
            delim = "\t")
